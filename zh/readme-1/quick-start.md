---
description: 从沙箱访问到沙箱开发、UAT 和生产上线的 Atlas API 集成快速入门指南。
---
...
如果您需要可导入的助手来获取工作流指导和故障排除，请使用 [Atlas AI 助手技能](integration-tools/atlas-ai-assistant-skill.md)。
...
<figure><img src="../.gitbook/assets/FlowChart_1_QuickStart (1).png" alt=""><figcaption><p>从沙箱访问到上线的端到端集成流程</p></figcaption></figure>
...
使用 [沙箱访问](making-requests.md) 进行此设置。
...
* [MCP 辅助开发](integration-tools/mcp-assisted-development.md)
...
使用 [UAT 验证](uat-submission-guide.md) 了解验证步骤。
...
使用 [生产上线](production-go-live.md) 获取上线检查清单。
...
1. 使用 [沙箱访问](making-requests.md) 获取凭证并确认请求基础。
2. 运行 [沙箱验证测试工具包](sandbox-development/sandbox-validation-test-kit.md) 进行快速的无代码健康检查。
3. 使用 [沙箱开发](sandbox-development/) 实现完整流程。
4. 沙箱流程稳定后进入 [UAT 验证](uat-submission-guide.md)。
5. 最后完成 [生产上线](production-go-live.md)。

如果您在实施过程中需要工作流查询、端点发现或故障排除的可选帮助，请使用 [集成工具](integration-tools/)。

### 首次端到端测试资源

如果您想先进行无代码环境检查，请使用 [沙箱验证测试工具包](sandbox-development/sandbox-validation-test-kit.md)。
...
如果您是全新开始，请继续 [沙箱访问](making-requests.md)。

如果沙箱访问已经完成，请继续 [沙箱开发](sandbox-development/)。

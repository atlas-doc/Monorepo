---
description: 中国 OTA 随单行李加购客户接入手册，涵盖信息透传、自动履约、异常确认与月底结算。
---
...
针对中国 OTA 在下单过程中随单销售行李的场景，Atlas 提供一套"行李信息透传 → 自动履约 → 异常确认 → 月底统一结算"的端到端能力，让 OTA 收单的行李订单可以稳定出票，并在月底按"多退少补"统一结算。
...
对于 **确认亏出** 的订单，若中国 OTA 后续申诉需要航司官网截图，**Atlas 不提供相关截图**。

客户应在收到亏出订单 **完成通知后**，自行前往航司官网查询订单并截图留存，以便后续申诉或内部核查。

### 月底统一结算

月底对账时，Atlas 统计所有中国 OTA 随单行李加购订单，并统一计算：
...
* **取消订单**：Atlas 终止该订单履约；
* **确认亏出**：继续出票，亏损月底结算。系统会为这类订单打标；**确认方式有两种**：① 调用 API（`confirmBaggageLoss.do`，传 `orderNo`）；② 在 atrip 页面点击确认（详见本文第三部分 §7）。
...
| # | 客户动作 | 是否必须 | 对应章节 |
|----|----|----|----|
| 1 | 在 `CreateOrder` 请求体中拼装 `passengerBaggages`，按乘机人 × 航段传入规格与价格 | 有行李时必须 | §3 接口与请求体 / §4 字段速查表 |
| 2 | 识别并处理错误码 `309`（规格不符）与 `3011`（航司报价高于 OTA 售价） | 必须 | §5 错误码与异常处理 |
| 3 | 配置钉钉群通知 + Webhook 回调地址接收异常通知 | 必须 | §6 通知与回调 |
| 4 | 实现"确认亏出 / 取消订单"的回传操作，并在 30 分钟内完成 | 必须 | §7 确认亏出 / 取消的回传 |
| 5 | 配合月底对账结算（盈余退还、亏损收取，多退少补） | 必须 | §8 结算对账 |
...
* [可选附加服务](../../booking/optional-ancillaries/README.md)
...
* [错误码总览](../../../../support-and-reference/troubleshooting-and-support/errors-handing/README.md)

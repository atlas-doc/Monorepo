---
description: >-
  瑞安航空（FR）特定预订流程，包括订单确认、价格透明度和儿童座位规则。
---
...
调用 [搜索](../../booking/booking-step-guides/search.md) 并保存选定的 `routingIdentifier`。
{% endstep %}

{% step %}
### 验价

调用 [验价](../../booking/booking-step-guides/verify.md) 并保存返回的 `sessionId`。
{% endstep %}

{% step %}
### 可选座位选择

如果您支持 FR 座位选择，请在预订前调用 [座位](../../booking/optional-ancillaries/seats-and-baggage/)。
...
使用乘客信息、联系方式和所需的 FR 字段调用 [创建订单](../../booking/booking-step-guides/create-order.md)。
...
然后调用 [确认订单](../../booking/booking-step-guides/confirm-order.md)。
...
仅在用户完成确认后，调用 [支付与出票](../../booking/booking-step-guides/payment-and-ticketing/)。
...
使用 [查询订单](../../booking/booking-step-guides/query-order/) 直到 `orderStatus=2` 且 `ticketStatus=0`。
{% endstep %}
{% endstepper %}
...
有关支持的 locale 值，请参阅 [语言环境参考](../../../support-and-reference/integration-reference/reference-data/locale.md)。
...
* [确认订单](../../booking/booking-step-guides/confirm-order.md)
* [支付与出票](../../booking/booking-step-guides/payment-and-ticketing/)
* [UAT 提交流程](../../../readme-1/uat-submission-guide.md)
* [特殊集成](./)

---
description: >-
  Atlas API webhook 设置、事件覆盖范围、投递期望及事件跟进指南。
---
...
{% hint style="info" %}
需要在 webhook、电子邮件和团队聊天中设置 ATRIP 通知？请从[多渠道通知](../multi-channel-notifications.md)开始。
{% endhint %}

如果您需要先进行渠道设置，请在阅读 webhook 事件详情之前打开[多渠道通知](../multi-channel-notifications.md)。
...
当您还需要通过电子邮件、钉钉、企业微信、Slack 或 Teams 进行 ATRIP 投递时，请使用[多渠道通知](../multi-channel-notifications.md)。
...
使用 [Webhook 注册与事件](../../../api-reference/webhook-and-incident-apis/webhook-registration-and-incidents.md) 在上线前保存 webhook URL。

如果您还使用邮箱或聊天投递，请在[多渠道通知](../multi-channel-notifications.md)中配置这些渠道。
{% endstep %}
...
1. 已出票：[出票完成通知](ticketing-complete-notification.md)
2. 已提交或更新作废：[作废通知](void-notification.md)
3. 航班日程变更：[日程变更通知](schedule-change-notification.md)
4. 航司状态更新：[航司状态更新通知](airline-status-update-notification.md)
5. 收到邮件：[电子邮件通知](email-notification.md)
{% endstep %}
...
1. 事件通知：[事件通知](incident-notification.md)
2. 事件查询：[事件查询](incident-query.md)
{% endstep %}
{% endstepper %}

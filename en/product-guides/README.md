---
description: >-
  Browse Atlas product guides by booking, post-booking, and extensions and
  integrations.
---

# Product Guides

Use this section to choose the right guide before you implement endpoint details.

New to the product? Read [What is Atlas API?](../what-is-atlas-api.md) for a concise capability overview.

### FAQ

#### What does Atlas API support?

Atlas API supports flight shopping, booking, payment, ticketing, post-booking operations, notifications, and supporting integrations.

Choose a capability below. Then use [API Reference](../api-reference/) for endpoint fields and schemas.

#### Which Atlas API booking path should I choose?

Choose Standard booking for the main search-to-order journey.

Choose Get Offer for known itineraries. Choose Fulfilment API when its offer-pricing flow fits your payment timing.

Use [Booking Overview](booking/booking-overview/) before implementation.

{% hint style="warning" %}
**🆕 New product:** Start with [🆕 Fulfilment API](booking/booking-flows/fulfillment-flow.md) when you need the `getOfferPrice.do` booking path, broader display rules, and immediate payment after order creation.

Use [Get Offer vs Fulfilment API](booking/booking-overview/booking-decisions/get-offer-vs-get-offer-price.md) when you are choosing between the two offer-based paths.
{% endhint %}

Browse Product Guides in three sections:

### Booking

Use this section for the full booking domain.

Start with [Booking](booking/).

If you want the newest booking path, start with [🆕 Fulfilment API](booking/booking-flows/fulfillment-flow.md).

Then use:

* [Booking Overview](booking/booking-overview/) for shared rules and identifiers
* [Booking paths](booking/booking-flows/) for end-to-end journey choice
* [Booking steps](booking/booking-step-guides/) for step-level booking pages
* [Optional ancillaries](booking/optional-ancillaries/) for optional seats and baggage

### Post-booking

Use this section for refunds, voids, order maintenance, and PNR follow-up.

Start with [Post-booking](post-booking/).

### Extensions & integrations

Use this section for notifications, data feeds, and special integrations.

Start with [Extensions & integrations](extensions-and-integrations/), [Webhook Overview](extensions-and-integrations/webhook-overview/), or [Flight Data Feed](extensions-and-integrations/flight-data-feed.md).

### Use API Reference when you need exact fields

Use [API Reference](../api-reference/) for exact schemas, parameters, and endpoint-level samples.

### Use Integration Guides when you need onboarding steps

Use [Integration Guides](../readme-1/) for sandbox setup, development, UAT, and production go-live.
